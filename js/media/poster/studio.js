...
window.drawPosterToCtx = window.drawPosterToCtx || function(ctx, W, H, post, options) {
  ...
  /* ── MARGO ghost wordmark (matches Lyric Back Poster) ── */
  ...
  ctx.restore();
  /* ── M-mark circle (matches Lyric Back Poster) ── */
  const pmSz = Math.round(Math.min(W,H)*0.07);
  const pbx = W-Math.round(W*0.036)-pmSz, pby = H-Math.round(H*0.034)-pmSz;
