...
export function drawExportAtmosphere(
  ...
  if (id === 'drift') {
    ...
    grad.addColorStop(0.45, 'rgba(232,197,71,0.14)')
    grad.addColorStop(0.55, 'rgba(232,197,71,0.08)')
  ...
  if (id === 'pulse') {
    ...
    ctx.fillRect(0, 0, w, h)
    ctx.strokeStyle = `rgba(232,197,71,${0.18 + hit * 0.5})`
    ctx.lineWidth = 1
  ...
  if (id === 'weight') {
    ...
    floorGrad.addColorStop(0, 'rgba(232,197,71,0.16)')
    floorGrad.addColorStop(1, 'rgba(232,197,71,0)')
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const dropH = h * 0.04
      ctx.fillStyle = `rgba(232,197,71,${0.35 * (1 - dropPhase)})`
      ctx.beginPath()
