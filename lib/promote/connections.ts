...
import { refreshTikTokAccessToken } from '@/lib/promote/tiktok-oauth'
...
export async function getValidTikTokAccessToken(
  ...
  if (!connection.refresh_token_enc) {
    await admin
      ...
      .update({ status: 'expired', last_error: 'TikTok refresh token missing — reconnect in Settings.' })
      .eq('id', connection.id)
    throw new Error('TikTok connection expired')
  }
  ...
  try {
    ...
    const refreshEnc = encryptPromoteToken(tokens.refresh_token)
