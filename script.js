...
f
irebasePosts.push({ firebaseId: child.key, ...child.val() }); 
}); 
f
irebasePosts.reverse(); 
const localPosts = JSON.parse(localStorage.getItem("margoPosts") || "[]"); 
const allPostsMap = new Map(); 
f
irebasePosts.forEach(post => allPostsMap.set(post.id, post)); 
...
${guessAttempts >= 3 ? `<button class="guess-btn reveal-btn" data-index="${index}" 
data-action="reveal">Reveal Answer</button>` : '} 
</div> 
`; 
} 
} else { 
...
guessResult.textContent = `
❌
 Not quite. ${remaining} attempts left before reveal option 
appears.`; 
} else { 
guessResult.textContent = "
❌
 Out of attempts. You can now reveal the answer from the 
post."; 
setTimeout(() => { 
modalController.close('guessModal'); 
renderFeed(); 
...
f
inalCanvas.width = dimensions.width; 
f
inalCanvas.height = dimensions.height; 
