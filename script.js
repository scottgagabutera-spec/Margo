...
showToast(`
⏳
 Wait ${remaining}s before posting again`); 
return; 
} 
...
console.warn("
⚠
 Firebase sync failed (but post is saved locally):", err); 
}); 
} 
...
const artistMatch = guessedArtist.includes(actualArtist) || 
...
break; 
case "facebook": 
window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}&quote=${
encodedText}`, "_blank"); 
break; 
case "telegram": 
...
break; 
case "reddit": 
window.open(`https://reddit.com/submit?url=${encodedUrl}&title=${encodedText}`, 
"_blank"); 
break; 
} 
shareModal.classList.add("hidden"); 
