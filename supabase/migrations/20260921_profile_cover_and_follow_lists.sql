-- Profile cover photo + independent follower/following list privacy.
-- Follows SELECT is limited to involved parties, so list pages use a
-- security-definer RPC that enforces profile visibility + list privacy.

alter table public.profiles
  add column if not exists cover_url text,
  add column if not exists follow_lists_private boolean not null default false;

comment on column public.profiles.cover_url is
  'Optional public cover photo URL (storage avatars/{id}/cover.*).';
comment on column public.profiles.follow_lists_private is
  'When true, only the owner can open follower/following lists. Independent of is_private.';
...
revoke all on function public.list_profile_follows(text, text, int, int) from public;
grant execute on function public.list_profile_follows(text, text, int, int) to anon, authenticated;

comment on function public.list_profile_follows(text, text, int, int) is
  'Returns accepted followers or following for a username, honoring profile visibility and follow_lists_private.';
