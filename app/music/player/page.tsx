...
function PlayerContent() {
  ...
  // ─── Next 3 songs in order, wrapping ───────────────────────────────
  const liveSongs = songs.filter(s => (s.status === 'live' || s.status === 'active') && s.audioUrl)
  const currentIndex = liveSongs.findIndex(s => s.id === songId)
  const nextSongs = Array.from({ length: 3 }, (_, i) => {
    const idx = (currentIndex + 1 + i) % liveSongs.length
    return liveSongs[idx]
  }).filter(Boolean).filter(s => s.id !== songId || liveSongs.length === 1)

  const primaryNext = nextSongs[0] || null

  const navigateToSong = useCallback((s: typeof primaryNext) => {
    if (!s) return
  ...
  const lyricsOpacity = trayOpen ? 0.15 : 1
  const lyricsPointer = trayOpen ? 'none' : 'auto'
  ...
  return (
    <div style={{ minHeight: '100vh', height: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      <style>{`
        ...
        @keyframes controls-fade-out { from { opacity: 1; transform: translateY(0) } to { opacity: 0; transform: translateY(8px) } }
        @keyframes controls-fade-in { from { opacity: 0; transform: translateY(8px) } to { opacity: 1; transform: translateY(0) } }
        ...
        .next-song-card { display: flex; align-items: center; gap: 14px; padding: 14px; border-radius: 16px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); cursor: pointer; transition: all 200ms ease; text-decoration: none; }
        .next-song-card:hover, .next-song-card:active { background: rgba(232,197,71,0.07); border-color: rgba(232,197,71,0.3); }
        .next-song-card.primary { background: rgba(232,197,71,0.06); border-color: rgba(232,197,71,0.25); }
        .tray-icon-btn { width: 36px; height: 36px; border-radius: 50%; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 150ms ease; color: rgba(255,255,255,0.6); font-size: 1rem; }
        .tray-icon-btn:hover { background: rgba(255,255,255,0.1); color: var(--text); }
      `}</style>
      ...
      {/* Lyrics viewport — fades when tray opens */}
      <div
        ref={viewportRef}
      ...
      {/* Tap hint — hidden when tray is open */}
      {!trayOpen && (
      ...
      {/* ── Up Next Tray — rises from bottom, replaces controls ── */}
      {trayOpen && (
        <div style={{
          position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 50,
          background: 'linear-gradient(to top, rgba(7,6,10,0.99) 0%, rgba(7,6,10,0.97) 60%, rgba(7,6,10,0.85) 100%)',
          backdropFilter: 'blur(24px)',
          borderTop: '1px solid rgba(232,197,71,0.12)',
          borderRadius: '24px 24px 0 0',
          padding: '20px 20px 44px',
          animation: 'tray-rise 420ms cubic-bezier(0.32,0.72,0,1) forwards',
        }}>
          {/* Tray top row — loop left, label center, dismiss right */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px' }}>
            <button
              className="tray-icon-btn"
              onClick={handleLoop}
              title="Loop this song"
              style={{ background: 'none', border: '1px solid rgba(255,255,255,0.1)' } as React.CSSProperties}
            >↺</button>

            <div style={{ textAlign: 'center' }}>
              {songEnded ? (
                <>
                  <p style={{ fontFamily: 'var(--font-lora), serif', fontSize: '0.52rem', fontWeight: 700, color: 'var(--text-3)', letterSpacing: '3px', textTransform: 'uppercase', margin: 0, marginBottom: '2px' }}>Song ended</p>
                  <p style={{ fontFamily: 'var(--font-lora), serif', fontStyle: 'italic', fontSize: '0.85rem', color: 'var(--text)', margin: 0 }}>{(song as any)?.title}</p>
                </>
              ) : (
                <p style={{ fontFamily: 'var(--font-lora), serif', fontSize: '0.52rem', fontWeight: 700, color: 'var(--gold)', letterSpacing: '3px', textTransform: 'uppercase', margin: 0, opacity: 0.85 }}>Up Next</p>
              )}
            </div>
            ...
            <button
              ...
              title="Dismiss"
              style={{ background: 'none', border: '1px solid rgba(255,255,255,0.1)', fontSize: '0.85rem' } as React.CSSProperties}
            >✕</button>
          </div>
              ...
              >
                ...
                <div style={{ position: 'relative', width: i === 0 ? '52px' : '44px', height: i === 0 ? '52px' : '44px', borderRadius: '10px', overflow: 'hidden', flexShrink: 0, transition: 'all 200ms ease' }}>
                  {s.artwork
                    ...
                    : <div style={{ width: '100%', height: '100%', background: 'rgba(232,197,71,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ color: 'var(--gold)', fontSize: '1rem', opacity: 0.5 }}>♪</span></div>
                  }
                </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{
                    ...
                    fontWeight: 600,
                    color: i === 0 ? 'var(--text)' : 'rgba(255,255,255,0.65)',
                ...
                <div style={{
                  width: i === 0 ? '32px' : '26px',
                  height: i === 0 ? '32px' : '26px',
                  borderRadius: '50%',
                  background: i === 0 ? 'var(--gold)' : 'rgba(255,255,255,0.06)',
                  border: i === 0 ? 'none' : '1px solid rgba(255,255,255,0.1)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: i === 0 ? '0.7rem' : '0.6rem',
                  color: i === 0 ? 'var(--bg)' : 'rgba(255,255,255,0.4)',
                  flexShrink: 0,
          ...
          {/* Auto-nav hint when song ended */}
          {songEnded && primaryNext && (
            <p style={{ fontFamily: 'var(--font-lora), serif', fontSize: '0.58rem', color: 'rgba(255,255,255,0.25)', textAlign: 'center', marginTop: '14px', letterSpacing: '1px' }}>
      ...
      {/* Bottom controls — hidden when tray is open */}
      {!trayOpen && (
        <footer style={{
          position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 40,
          background: 'linear-gradient(to top, var(--bg) 75%, transparent)',
          padding: '20px 24px 36px',
          animation: 'controls-fade-in 300ms ease forwards',
        }}>
