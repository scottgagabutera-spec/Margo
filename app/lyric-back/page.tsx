...
function LyricBackContent() {
  ...
  const handleSearch = useCallback(async (value: string) => {
    ...
    try {
      ...
      setSearchResults((data.results || []).map((r: any) => ({
        id: String(r.id || r.song), title: r.song, artist: r.artist,
        artwork: r.artwork || '', source: r.source as Source,
      })))
  ...
  const handleSelectSong = useCallback((result: SearchResult) => {
    setSelectedSong(result); setArtistName(result.artist)
    setSongName(result.title); setShowResults(false); setStep(2)
  }, [])
  ...
  const handleVibeSelect = useCallback((vibe: Vibe) => {
    setSelectedVibe(vibe); setStep(4)
  }, [])
  ...
  /* ─── promote + reply — navigate immediately, write in background ── */
  const promoteAndReply = (echo: typeof echoes[0]) => {
    // Navigate first — don't block the user on the Firebase write
  ...
  return (
    <main style={{ minHeight: '100vh', background: bg, position: 'relative' }}>
      ...
      <div style={{ paddingTop: '120px', paddingBottom: '80px', paddingLeft: '24px', paddingRight: '24px' }}>
        <div style={{ maxWidth: '640px', margin: '0 auto' }}>
          ...
          <div style={{
            ...
            borderRadius: '20px', padding: '28px 24px', marginBottom: '40px',
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', position: 'relative', zIndex: 1 }}>
              {respondingTo?.username && (
                <span style={{ fontFamily: font, fontSize: '0.72rem', color: text3 }}>by {respondingTo.username}</span>
              )}
              {respondingTo?.emotion && (
                <span style={{ fontFamily: font, fontSize: '0.6rem', fontWeight: 700, color: gold, letterSpacing: '1px', textTransform: 'uppercase', padding: '4px 12px', background: 'rgba(232,197,71,0.12)', border: `1px solid ${goldBorder}`, borderRadius: '50px' }}>
                  {respondingTo.emotion}
                </span>
              )}
            </div>
          ...
          <div style={{ display: step === 1 ? 'block' : 'none' }}>
            <div style={{ textAlign: 'center', marginBottom: '40px' }}>
              <h1 style={{ fontFamily: font, fontStyle: 'italic', fontSize: '2rem', color: gold, marginBottom: '8px' }}>Find your lyric back</h1>
              <p style={{ fontFamily: font, fontSize: '0.82rem', color: text3 }}>Search by lyric, song, or artist</p>
            </div>
            <div style={{ position: 'relative' }}>
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  style={{ width: '100%', height: '60px', paddingLeft: '52px', paddingRight: '24px', background: goldFaint, border: `1px solid ${goldBorder}`, borderRadius: '16px', color: text, fontSize: '1rem', fontFamily: font, outline: 'none', boxSizing: 'border-box' }}
                />
              </div>
              {showResults && (
                <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '8px', background: surface, border: `1px solid ${border}`, borderRadius: '16px', overflow: 'hidden', zIndex: 50 }}>
                  {searchLoading && (
                    <div style={{ textAlign: 'center', padding: '16px', fontFamily: font, color: gold, fontSize: '0.82rem', fontStyle: 'italic' }}>Searching…</div>
                  )}
                  {searchResults.map(result => (
                    <button key={result.id} onClick={() => handleSelectSong(result)}
                      style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', transition: 'background 150ms ease' }}
                      onMouseEnter={e => (e.currentTarget.style.background = goldFaint)}
                      onMouseLeave={e => (e.currentTarget.style.background = 'none')}
                    >
                      {result.artwork && <img src={result.artwork} alt={result.title} style={{ width: '44px', height: '44px', borderRadius: '8px', objectFit: 'cover', flexShrink: 0 }} />}
          ...
          <div style={{ display: step === 2 ? 'block' : 'none' }}>
            <div style={{ textAlign: 'center', marginBottom: '36px' }}>
              <h1 style={{ fontFamily: font, fontStyle: 'italic', fontSize: '2rem', color: gold, marginBottom: '8px' }}>Your lyric back</h1>
              <p style={{ fontFamily: font, fontSize: '0.82rem', color: text3 }}>Enter the lyric that says it back</p>
            </div>
            ...
            <div style={{ background: goldFaint, border: `1px solid ${goldBorder}`, borderRadius: '20px', padding: '28px', position: 'relative', overflow: 'hidden' }}>
              ...
              <textarea value={lyric} onChange={e => setLyric(e.target.value.slice(0, 140))}
                ...
                style={{ width: '100%', background: 'transparent', fontSize: '1.4rem', fontFamily: font, fontStyle: 'italic', color: gold, textAlign: 'center', lineHeight: 1.6, border: 'none', outline: 'none', resize: 'none', position: 'relative', zIndex: 10, boxSizing: 'border-box' }}
              />
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '14px', position: 'relative', zIndex: 10 }}>
                ...
                <button onClick={handleLyricComplete} disabled={lyric.trim().length === 0}
                  style={{ padding: '10px 24px', background: gold, color: bg, borderRadius: '50px', fontFamily: font, fontWeight: 700, fontSize: '0.6rem', letterSpacing: '1px', textTransform: 'uppercase', border: 'none', cursor: 'pointer', opacity: lyric.trim().length === 0 ? 0.4 : 1, transition: 'opacity 150ms ease' }}
                >Continue</button>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: step === 3 ? 'block' : 'none' }}>
            <div style={{ textAlign: 'center', marginBottom: '28px' }}>
              <h1 style={{ fontFamily: font, fontStyle: 'italic', fontSize: '2rem', color: gold, marginBottom: '8px' }}>How does it feel?</h1>
              <p style={{ fontFamily: font, fontSize: '0.82rem', color: text3 }}>Pick the vibe, or skip</p>
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '10px', marginBottom: '20px' }}>
              {VIBES.map(vibe => (
                <button key={vibe} onClick={() => handleVibeSelect(vibe)}
                  style={{ padding: '10px 20px', borderRadius: '50px', fontFamily: font, fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer', transition: 'all 150ms ease', background: selectedVibe === vibe ? gold : 'transparent', color: selectedVibe === vibe ? bg : gold, border: selectedVibe === vibe ? `1px solid ${gold}` : `1px solid ${goldBorder}` }}
                >{VIBE_LABELS[vibe]}</button>
              ))}
            </div>
            <div style={{ textAlign: 'center' }}>
              <button onClick={() => setStep(4)} style={{ background: 'transparent', border: 'none', fontFamily: font, fontSize: '0.82rem', color: text3, cursor: 'pointer', textDecoration: 'underline' }}>Skip — no vibe</button>
            </div>
          </div>
          ...
          <div style={{ display: step === 4 ? 'block' : 'none' }}>
            <div style={{ textAlign: 'center', marginBottom: '28px' }}>
              <h1 style={{ fontFamily: font, fontStyle: 'italic', fontSize: '2rem', color: gold, marginBottom: '8px' }}>Ready to send it back?</h1>
              <p style={{ fontFamily: font, fontSize: '0.82rem', color: text3 }}>Your lyric back is set to go</p>
            </div>
            <div style={{ background: goldFaint, border: `1px solid ${goldBorder}`, borderRadius: '20px', padding: '32px', marginBottom: '28px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
              ...
              {selectedVibe && (
                <span style={{ display: 'inline-block', padding: '5px 14px', background: 'rgba(232,197,71,0.12)', border: `1px solid ${goldBorder}`, borderRadius: '50px', fontFamily: font, fontSize: '0.6rem', fontWeight: 700, color: gold, letterSpacing: '1px', textTransform: 'uppercase', position: 'relative', zIndex: 1 }}>
                  {VIBE_LABELS[selectedVibe]}
                </span>
              )}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxWidth: '320px', margin: '0 auto' }}>
              <button onClick={() => handlePost(false)}
                style={{ padding: '15px 28px', background: gold, color: bg, borderRadius: '50px', fontFamily: font, fontWeight: 700, fontSize: '0.6rem', letterSpacing: '1px', textTransform: 'uppercase', border: 'none', cursor: 'pointer', boxShadow: '0 6px 28px rgba(232,197,71,0.28)' }}
              >Send It</button>
              <button onClick={() => handlePost(true)}
                style={{ padding: '13px 28px', background: 'transparent', color: text2, border: '1px solid var(--border-hi)', borderRadius: '50px', fontFamily: font, fontWeight: 600, fontSize: '0.6rem', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer' }}
              >Keep Private</button>
            </div>
          </div>
              ...
              {echoes.map(lb => {
                ...
                return (
                  <div
                    key={lb.id}
                    style={{
                      background: 'linear-gradient(135deg, rgba(232,197,71,0.03) 0%, rgba(255,255,255,0.01) 100%)',
                      border: `1px solid ${border}`,
                      borderLeft: '3px solid rgba(232,197,71,0.25)',
                      borderRadius: '16px',
                      padding: '20px 22px',
                    ...
                    onMouseLeave={e => {
                      ...
                      e.currentTarget.style.borderLeftColor = 'rgba(232,197,71,0.25)'
                    }}
                  >
                    {/* Username + emotion */}
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                      ...
                      {lb.emotion && (
                        <span style={{ fontFamily: font, fontSize: '0.58rem', fontWeight: 700, color: gold, letterSpacing: '1px', textTransform: 'uppercase', padding: '3px 10px', background: 'rgba(232,197,71,0.08)', border: `1px solid ${goldBorder}`, borderRadius: '50px' }}>
                          {lb.emotion}
                        </span>
                      )}
                    </div>
                    ...
                    {/* Lyric */}
                    <p style={{ fontFamily: font, fontStyle: 'italic', fontSize: '1.1rem', color: text, lineHeight: 1.6, marginBottom: '6px' }}>
                      &ldquo;{lb.lyric}&rdquo;
                    </p>
                    <p style={{ fontFamily: font, fontSize: '0.75rem', color: text3, marginBottom: '16px' }}>
                      — {lb.artist}, {lb.song}
                    </p>

                    {/* Actions — Resonate (primary left) · Lyric Back + Card (right) */}
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '14px', borderTop: `1px solid ${border}` }}>
                      ...
                      <button
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: '7px', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: font, color: hasResonated ? gold : text3, transition: 'color 150ms ease' }}
                      >
                      ...
                      {/* Lyric Back + Card — right side, secondary/tertiary */}
                      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                        <button
                          onClick={() => promoteAndReply(lb)}
                          style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: font, color: text3, transition: 'color 150ms ease' }}
                          onMouseEnter={e => (e.currentTarget.style.color = gold)}
                          onMouseLeave={e => (e.currentTarget.style.color = 'var(--text-3)')}
                        >
                          <MessageCircle style={{ width: '15px', height: '15px', flexShrink: 0 }} />
                          <span style={{ fontSize: '0.6rem', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>Lyric Back</span>
                        </button>
                        ...
                        <button
                          onClick={() => {
                            setCardData({
                              ...
                              // Pass parent so the card canvas draws the full conversation
                          ...
                          onMouseLeave={e => (e.currentTarget.style.color = 'var(--text-3)')}
                        >
                          <CreditCard style={{ width: '15px', height: '15px', flexShrink: 0 }} />
                          <span style={{ fontSize: '0.6rem', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>Card</span>
                        </button>
                      </div>
                    </div>
                  </div>
      ...
      {/* Card export — passes parent data for dual-card canvas */}
      {showCard && (
        <CardExportModal
          open={showCard}
